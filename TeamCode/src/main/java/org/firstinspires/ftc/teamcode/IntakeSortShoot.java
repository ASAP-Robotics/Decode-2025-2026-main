...
/**
 * FULL SYSTEM (No Limelight, No Turret)
 * - Driver toggles intake (A). Intake runs FWD until 3 balls then REV to reject extras (with hysteresis). Pauses during a code run.
 * - Magazine color sorter at INTAKE with 3 slots (120° apart). Shooter FEEDER is 240° CW from intake (2 steps offset).
 * - Trigger (RT) requests ONE shot. Flywheel only spins when a shot is requested or a feed pulse is finishing.
 * - Code sequencer for PPG / PGP / GPP: press B to arm/run, D-pad Up cycles codes. Each trigger press advances one step.
 * - No camera, no turret. RPM is manual via presets + fine/coarse nudges.
 */
...
public class IntakeSortShoot extends LinearOpMode {
 ...
 //   private boolean lastB = false;           // B edge (intake toggle)
    ...
    private enum FeedState { IDLE, OPEN, COOLDOWN }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        try { intake = hardwareMap.get(DcMotorEx.class, "intake"); } catch (Exception ignored) { intake = null; }
        ...
        } catch (Exception ignored) { sorter = null; }
        ...
        time.reset();
        telemetry.addLine("Ready | B=toggle intake | RT=toggle flywheel | LB=start code | D-pad Up=cycle code | X=short Y=mid RB=long | D-pad L/R=±25, D/U=±100 RPM");
        telemetry.update();
        waitForStart();
        ...
        while (opModeIsActive()) {
           ...
           // boolean bNow = gamepad1.b;
            ...
            if (lbNow && !seqActive) { seqActive = true; seqStep = 0; }
            lastDpadUp = upNow;
            ...
            if (upNow && !lastDpadUp) { /* already used to cycle code; ignore coarse + */ }
            ...
            boolean rpmReady = Math.abs(getCurrentRpm() - (keepSpinning ? targetRpm : 0)) <= CFG.FLYWHEEL_RPM_TOL && (keepSpinning ? targetRpm > 100 : true);
            ...
            // ===== TELEMETRY =====
            telemetry.addData("code", codes[codeIndex] + (seqActive ? " [RUN] step=" + seqStep : " [ARMED]"));
    ...
    static class BallSorter {
        ...
        BallSorter(DistanceSensor range, ColorSensor color, Servo mag, double[] slotPositions, int feederOffsetSteps) {
            this.range = range;
            this.color = color; this.mag = mag; this.positions = slotPositions.clone();
        ...
        int feederIndex(){ return (currentSlot + feederOffsetSteps) % 3; }

        /** Color currently under the FEEDER. */
        String feederColor(){ return slots[feederIndex()]; }

        /** True if desired color is under FEEDER now. */
        boolean feederHas(String wanted){ return wanted != null && wanted.equals(feederColor()); }
        ...
        void seekColorAtFeeder(String wanted){
            ...
            for (int i = 1; i <= 2; i++){
                ...
                if (wanted.equals(slots[candidateFeeder])) { stepToNextSlot(); break; }
            }
        }

        /** After shooting the FEEDER slot, clear it. */
        void consumeFeeder(){ slots[feederIndex()] = null; }

        /** Number of non-empty slots currently stored. */
        int ballCount(){ int c = 0; for (String s : slots) if (s != null) c++; return c; }
    }
}
