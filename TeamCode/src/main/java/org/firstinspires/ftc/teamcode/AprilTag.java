 ...
 * AprilTagLocator
 * ---------------
 * Drop-in helper for FTC AprilTags using VisionPortal + AprilTagProcessor.
 *
 * How to use (in your OpMode):
 * ----------------------------
 *  Position camPos = new Position(DistanceUnit.INCH, -5, 7, 12, 0); // <-- set for YOUR mount
 *  YawPitchRollAngles camOri = new YawPitchRollAngles(AngleUnit.DEGREES, 0, -90, 0, 0);
 *
 *  AprilTagLocator tags = new AprilTagLocator(hardwareMap, true, "Webcam 1", camPos, camOri);
 *  tags.start();                  // call in init()
 *  waitForStart();
 *  while (opModeIsActive()) {
 *      tags.update();             // call every loop
 *      if (tags.isTagVisible()) {
 *          telemetry.addData("ID", tags.getLastSeenId());
 *          telemetry.addData("XYZ (in)", "%.1f  %.1f  %.1f", tags.getX(), tags.getY(), tags.getZ());
 *          telemetry.addData("PRY (deg)", "%.1f  %.1f  %.1f",
 *                  tags.getPitch(), tags.getRoll(), tags.getYaw());
 *      } else {
 *          telemetry.addLine("No tag visible");
 *      }
 *      telemetry.addData("Visible IDs", tags.getLastVisibleIds());
 *      telemetry.update();
 *  }
 *  tags.stop();                   // call when done
 *
 * Filtering (optional):
 * ---------------------
 *  tags.setAllowedIds(Set.of(1,2,3)); // only consider these IDs (null/empty = any)
 *  tags.setSelectedId(2);             // lock to a single ID (null to clear)
 *  // Or cycle at runtime with your buttons, using getLastVisibleIds() to choose.
 */
public class AprilTag {
    ...
    public void pause()  { if (visionPortal != null) visionPortal.stopStreaming(); }
    public void resume() { if (visionPortal != null) visionPortal.resumeStreaming(); }
    ...
    /**
     * Call every loop. Updates visibility and pose cache.
     * Selection priority: selectedId (if set) → tag with metadata, nearest by range (if available) → first match.
     */
    public void update() {
        tagVisible = false;
        lastSeenId = -1;
        ...
        if (selectedId != null) {
            for (AprilTagDetection d : detections) {
                if (passesFilter(d) && d.id == selectedId) { best = d; break; }
            }
        }
    ...
    public int getLastSeenId()    { return lastSeenId; }
