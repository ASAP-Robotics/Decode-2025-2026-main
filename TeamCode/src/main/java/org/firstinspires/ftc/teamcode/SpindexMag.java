...
public class SpindexMag {
    ...
    public static final int NULL = -1; // /< sort of like null (intentionally doesn't contain a useful value)
    ...
    private final double liftServoRestPos = 0.0; // /< the position of the lift servo when at rest | TODO: tune
    private final double liftServoShootPos = 0.3; // /< the position of the lift servo when shooting | TODO: tune
    ...
    public SpindexMag(ActiveIntake intake, Servo spinServo, Servo liftServo, ColorSensor colorSensor) {
        mag_intake = intake;
     ...
     * @return the index in the spindex where that color is located, or -1 if that color isn't in the mag
     */
